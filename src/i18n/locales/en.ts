/** English UI chrome. App chrome only — never the testee's entered content. */
export const en = {
  appName: 'Repertory Grid',
  nav: {
    language: 'Language',
    theme: 'Toggle dark mode',
  },
  start: {
    tagline: 'Role Construct Repertory Test by G. Kelly',
    start: 'Start',
    resume: 'Resume the saved test',
  },
  common: {
    back: 'Back',
    next: 'Next',
  },
  // Header title per flow (shown top-left in place of the app name).
  flow: {
    characters: 'Characters',
    constructs: 'Constructs',
    grid: 'Grid',
    grid10: '10×10 Grid',
    ranking: 'Constructs ranking',
  },
  names: {
    progress: 'Names',
    hint: 'tap any card to edit',
    placeholder: 'a name…',
    finish: 'Finish',
    complete: 'All names entered.',
    continue: 'Continue to the comparisons',
  },
  elicit: {
    progress: 'Construct',
    pickOdd: 'Which two are alike? Tap the one that stands apart.',
    alike: 'alike',
    different: 'the opposite',
    emergentPole: 'elicited pole',
    contrastPole: 'contrast pole',
    emergentQ: 'What do {{a}} and {{b}} share?',
    contrastQ: 'Then {{c}} is the opposite — how?',
    emergentPlaceholder: 'shared trait…',
    contrastPlaceholder: 'the opposite…',
    whoElse: 'Who else is «{{trait}}»? Select everyone who fits.',
    whoElseGeneric: 'Who else fits? Select everyone who shares it.',
    finish: 'Finish',
  },
  result: {
    export: 'Export PDF',
    emergentCol: 'elicited pole',
    contrastCol: 'contrast pole',
    names: 'characters',
    classes: 'classes',
    roles: 'roles',
    legendMatch: 'elicited trait',
    legendContrast: 'opposite trait',
    legendTriad: 'one of the three compared',
    legendPos: 'shared (+1)',
    legendNeg: 'opposed (-1)',
    legendEmergent: 'emergent',
    legendContrast2: 'contrast',
    // Character-column groups: the merged "groups" row above the names + the names-row tints.
    groups: 'groups',
    groupMe: 'me',
    groupFamily: 'family',
    groupClose: 'close ones',
    groupSituational: 'situational characters',
    groupRelations: 'relationships',
    groupAuthority: 'authority',
    groupValues: 'values',
  },
  tables: {
    menu: 'Tables',
    newTable: 'New table',
    defaultName: 'Main table',
    relationshipsName: 'Character relationships',
    diagramName: 'Constructs relations diagram',
    relconstructsName: 'Constructs relations table',
    grid10Name: '10×10 grid',
    rhoName: 'ρ',
    rho2Name: 'ρ² × 100',
    default: 'default',
    readOnly: 'read-only',
    allChars: 'all {{n}} characters',
    charCount: '{{n}} characters',
    rename: 'Rename',
    delete: 'Delete',
    newTitle: 'New custom table',
    newSub: 'Choose the characters to compare, then name the table.',
    renameTitle: 'Rename table',
    renameSub: 'Give this table a new name.',
    nameLabel: 'Table name',
    namePlaceholder: 'Close family',
    charsLabel: 'Characters — pick at least 2',
    selected: '{{n}} selected',
    needTwo: '{{n}} selected — pick at least 2',
    save: 'Save table',
    cancel: 'Cancel',
    untitled: 'Table {{n}}',
    deleteTitle: 'Delete table',
    deleteBody: 'Delete «{{name}}»? This can’t be undone.',
    confirmDelete: 'Delete',
    exportTitle: 'Export to PDF',
    exportSub: 'Choose which tables to include — combined into one document, one table per page.',
    exportSelected: '{{n}} of {{total}} selected',
    generate: 'Generate PDF',
  },
  pairs: {
    charA: 'Character…',
    charB: 'Character…',
    search: 'Search…',
    noMatch: 'No matches',
    onGrid: 'show on grid',
    shownOnGrid: 'Highlighted on the grid',
    remove: 'Remove pair',
    add: 'Add pair',
    pdfHeading: 'Character pairs',
  },
  reset: {
    button: 'Start over',
    close: 'Close',
    cancel: 'Cancel',
    continue: 'Continue',
    confirm: 'Yes, start over',
    warnBody: 'Starting over will wipe all your current progress and reset the test. Do you wish to start the test over?',
    confirmBody: 'This action is irreversible, all progress will be lost. Are you sure you wish to start over?',
  },
  sharing: {
    // Share button — the leading dot is the coloured indicator, rendered separately.
    modeSilent: 'Sharing mode: OFF',
    modeListening: 'Sharing mode: Listening...',
    modeBroadcasting: 'Sharing mode: Broadcasting',
    ariaButton: 'Session sharing',
    // Sharing popup (opened from the button)
    title: 'Sharing',
    enable: 'Share this session',
    subtitle: 'Enable real-time session sharing. The observer will have read-only access.',
    linkLabel: 'Observer link',
    linkHint: 'Click to copy, then send it to your observer.',
    copied: 'Copied!',
    observers: 'Observers connected: {{count}}',
    waitingForObserver: 'Waiting for an observer to join…',
    close: 'Close',
    // Approval popup (testee)
    approveTitle: 'Let an observer in?',
    approveBody: 'Someone opened your link and wants to watch this session in read-only mode.',
    approveYes: 'Allow',
    approveNo: 'Deny',
    // Observer screen
    connecting: 'Connecting…',
    waitingTitle: 'Waiting for the host',
    waitingBody: 'The host has been asked to let you in. This will update on its own.',
    live: 'Live',
    observerNote: 'Watching a shared session — read only',
    rejectedTitle: 'Denied',
    rejectedBody: 'The host chose not to share this session with you.',
    endedTitle: 'Sharing ended',
    endedBody: 'The host has stopped sharing this session.',
    ok: 'OK',
    leave: 'Leave broadcast',
    leaveConfirmBody: 'Are you sure you want to leave this broadcast?',
    leaveConfirmYes: 'Yes, leave',
    cancel: 'Cancel',
    cancelRequest: 'Cancel request',
    cancelConfirmBody: 'Are you sure you want to cancel your request?',
    cancelConfirmYes: 'Yes',
    cancelConfirmNo: 'No',
    canceledNotice: 'Join request was canceled by the sender',
    observerJoined: 'An observer joined the session.',
    observerLeft: 'An observer left the session.',
  },
  save: {
    button: 'Save',
    title: 'Save and resume later',
    info: "You can resume your test later from this point by saving the link below or downloading a save file. If you'll make some more progress after the save, open this dialog again and re-save a new link or a save file.",
    linkLabel: 'save and resume link',
    hint: 'click to copy',
    close: 'Close',
    generating: 'Generating link…',
    error: "Couldn't reach the server. Download a save file below instead.",
    download: 'Download a save file',
  },
  resumeDlg: {
    title: 'Resume a saved test',
    linkLabel: 'resume link',
    linkPlaceholder: 'paste your resume link',
    clear: 'Clear',
    go: 'Resume',
    or: 'or',
    dropText: 'Drag and drop a save file into this area:',
    selectFile: 'Select a save file',
    removeFile: 'Remove',
    error: "That link or file couldn't be read.",
    // Two-source confirm (a link and a save file are both provided).
    twoSources:
      'You have provided 2 sources of saved test progress. Which one you want to restore the progress from?',
    useLink: 'Link',
    useFile: 'Save file',
  },
  // The optional 3rd/4th flows — distilling the 22×22 into a focused 10×10 grid, then ranking.
  g10: {
    create: 'Create 10×10 grid',
    ranking: 'Constructs ranking',
    rankingLocked: 'Build the 10×10 grid first.',
    eleventh: '11th construct',
    step1: 'Step 1 · Characters',
    step2: 'Step 2 · Constructs',
    step3: 'Step 3 · Constructs ranking',
    charsTitle: 'Pick 10 characters',
    charsHint: 'Choose the 10 characters for the concentrated grid. “Me” is always included.',
    groupTitle: 'Reduce to 10 constructs',
    groupHint:
      'Drag a construct from the middle onto a Keep row to merge them (then name the merged pair’s new poles), onto “New row” to start a row, or into “Throw away” to drop it. Keep exactly 10 rows.',
    keepCount: 'Keep: {{n}} / 10',
    keepFull: 'The "Keep" column is full, it cannot house more than 10 new constructs',
    listFull: 'The list is full!',
    existing: 'Existing constructs',
    newRow: 'Drop here to start a new row',
    throwAway: 'Throw away',
    newEmergent: 'new elicited pole…',
    newContrast: 'new contrast pole…',
    elicitTitle: 'Build the 10×10',
    pickTriad: 'Pick 3 characters for the triad',
    pickConstruct: 'Which construct is this?',
    rankTitle: 'Rank the characters',
    rankHint:
      'Drag the characters into order for this construct — most like the elicited pole at the top, most like the contrast pole at the bottom.',
    goodPole: 'good',
    badPole: 'bad',
    gridEmptyNote: 'The 10×10 grid hasn’t been built yet.',
    rankEmptyNote: 'The constructs ranking hasn’t been completed yet.',
  },
}

/** Shape all locales conform to (string values, so translations are accepted). */
export type Translation = typeof en
